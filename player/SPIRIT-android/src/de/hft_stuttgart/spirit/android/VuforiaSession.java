...
public class VuforiaSession implements UpdateCallbackInterface {
	...
	public void initAR(Activity activity, int screenOrientation) {
		...
		if (mInitVuforiaTask != null) {
			...
			vuforiaException = new VuforiaException(
					VuforiaException.VUFORIA_ALREADY_INITIALIZATED, logMessage);
		...
		if (vuforiaException == null) {
			try {
				...
				vuforiaException = new VuforiaException(
						VuforiaException.INITIALIZATION_FAILURE, logMessage);
				Log.e(LOGTAG, logMessage);
			}
		}
	...
	private class LoadTrackerTask extends AsyncTask<Void, Integer, Boolean> {
		...
		protected void onPostExecute(Boolean result) {
			...
			if (!result) {
				...
				Log.e(LOGTAG, logMessage);
				vuforiaException = new VuforiaException(
						VuforiaException.LOADING_TRACKERS_FAILURE, logMessage);
			} else {
	...
	private boolean setFocusMode(int mode) throws VuforiaException {
		...
		if (!result)
			throw new VuforiaException(VuforiaException.SET_FOCUS_MODE_FAILURE,
	...
	private void configureVideoBackground() {
		...
		// ---
//		config.setPosition(new Vec2I(mScreenWidth / 2, mScreenHeight / 2));
		// ---
