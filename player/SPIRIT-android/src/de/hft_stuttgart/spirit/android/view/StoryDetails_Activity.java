...
/**
 * StoryDetails_Activity is a activity to display the details of a story and
 * download/play the story.
 * 
 * @author Oliver
 *
 */
public class StoryDetails_Activity extends ActionBarActivity {
	...
	/**
	 * Get the informations about a story from a intent and display the
	 * informations in the activity.
	 */
	@Override
	protected void onStart() {
		...
		if (intent.hasExtra(EXTRA_UPDATEDAT)) {
			...
			textv.setText("#UNDEF");
			logmessage += "Set creation date by default to \""
					+ textv.getText() + "\"\n";
		}
		...
		if (intent.hasExtra(EXTRA_LOCATION)) {
			...
			try {
				...
				Marker markr = map.addMarker(new MarkerOptions()
						.position(point).title("Position"));
				map.animateCamera(CameraUpdateFactory.newLatLngZoom(point, 16));
				logmessage += "Set creation date by intent to \""
						+ markr.getPosition().toString() + "\"\n";
			} catch (Exception e) {
				// This error may be thrown because the smartphone/emulator
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (getIntent().hasExtra(EXTRA_STOREORINSTALLED)) {
			if (getIntent().getStringExtra(EXTRA_STOREORINSTALLED).equals(
					"STORE")) {
			...
			if (getIntent().getStringExtra(EXTRA_STOREORINSTALLED).equals(
				...
				File saveGame = new File(
						Environment.getExternalStorageDirectory()
								+ "/StorytellAR/Content/"
								+ intent.getIntExtra(EXTRA_STORYID, -1)
								+ "/save.ser");
				if (saveGame.exists() && !saveGame.isDirectory()) {
					menu.getItem(0).setTitle("Fortsetzen");
					menu.getItem(1).setEnabled(true); // shows restart Menu
														// option
				} else {
					menu.getItem(0).setTitle("Start");
					menu.getItem(1).setEnabled(false); // hides restart Menu
														// option
				}
				return true;
			}
	...
	/**
	 * Handle clicks on the start, restart, delete and download buttons. On
	 * click of the start button, the story is started in spirit. On click of
	 * the restart button, a old savestate of the story is started. On click of
	 * the delete button, the local files are removed from the device. On click
	 * of the download button, the download of the story from the server is
	 * started.
	 *
	 */
	@Override
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_start:
			...
			start_intent.putExtra(EXTRA_STORYID,
					intent.getIntExtra(EXTRA_STORYID, -1));
			start_intent.putExtra("EXTRA_RESTARTMARKER", "false");
			startActivity(start_intent);
			return true;
		case R.id.action_restart:
			Intent restart_intent = new Intent(getApplicationContext(),
					AndroidLauncher.class);
			restart_intent.putExtra(EXTRA_STORYID,
					intent.getIntExtra(EXTRA_STORYID, -1));
			restart_intent.putExtra("EXTRA_RESTARTMARKER", "true");
			startActivity(restart_intent);
			return true;
		case R.id.action_delete:
			toast = Toast.makeText(getApplicationContext(), "Delete Story "
					+ intent.getStringExtra(EXTRA_STORYNAME) + " (ID: "
					+ intent.getIntExtra(EXTRA_STORYID, -1) + ")",
					Toast.LENGTH_SHORT);
			toast.show();
			ContentDownloader.getInstance().deleteStory(
					intent.getIntExtra(EXTRA_STORYID, -1));
			Intent i_delete = new Intent(getApplicationContext(),
					Main_Activity.class);
			if (intent.hasExtra("StoreFragmentStoryFilter")) {
				i_delete.putExtra("StoreFragmentStoryFilter",
						intent.getParcelableExtra("StoreFragmentStoryFilter"));
			}
			if (intent.hasExtra("InstalledFragmentStoryFilter")) {
				i_delete.putExtra("InstalledFragmentStoryFilter", intent
						.getParcelableExtra("InstalledFragmentStoryFilter"));
			}
		...
		case R.id.action_download:
			...
			builder.setTitle("Jetzt die vollen "
					+ intent.getStringExtra(EXTRA_SIZE)
					+ " MB für die Story herunterladen?");
					...
					new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							Toast.makeText(
									...
									"Download Story "
											+ intent.getStringExtra(EXTRA_STORYNAME)
											+ " (ID: "
											+ intent.getIntExtra(EXTRA_STORYID,
													-1) + ")",
									Toast.LENGTH_SHORT).show();
							DownloadStory task = new DownloadStory();
							task.execute();
							Intent i_download = new Intent(
									getApplicationContext(),
									Main_Activity.class);
							if (intent.hasExtra("StoreFragmentStoryFilter")) {
								i_download
										.putExtra(
												"StoreFragmentStoryFilter",
												intent.getParcelableExtra("StoreFragmentStoryFilter"));
							}
							if (intent.hasExtra("InstalledFragmentStoryFilter")) {
								i_download.putExtra(
										"InstalledFragmentStoryFilter",
										intent.getParcelableExtra("InstalledFragmentStoryFilter"));
							}
	...
	public class DownloadStory extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			Intent intent = getIntent();
			ContentDownloader.getInstance().downloadStory(
					intent.getIntExtra(EXTRA_STORYID, -1));
			return null;
		}
